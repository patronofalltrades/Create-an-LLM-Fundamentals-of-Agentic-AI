...
const checkpointBytes = fs.readFileSync(path.join(__dirname,'results/expanded-evals/checkpoint.json'));
...
const probe=JSON.parse(fs.readFileSync(path.join(__dirname,'results/expanded-evals/inspection.json')));
...
assert.ok(imported.tokens.includes('cold'));
...
console.log(`Pooled PCA retains ${(fit.retained*100).toFixed(4)}% variance; cold neighbors after training:`,nearest(bundle.after,bundle.tokens.indexOf('cold')).map(x=>[bundle.tokens[x.i],x.score]));
